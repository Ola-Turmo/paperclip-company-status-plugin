...
export function computeHealthScore(params) {
    ...
    if (params.loopDetected)
        score -= HEALTH_WEIGHTS.loopPenalty;
    ...
    if (params.websiteHealthy)
        score += HEALTH_WEIGHTS.websiteBonus;
...
export async function checkWebsite(ctx, company) {
    const meta = company.metadata;
    const url = meta?.websiteUrl;
    if (!url)
...
export async function fetchAgentRuns(ctx, agentId, companyId, days) {
    try {
        const since = new Date(nowTs() - days * DAY_MS).toISOString();
        const rows = await ctx.db.query(`SELECT COUNT(*) as count FROM heartbeat_runs WHERE agent_id = $1 AND company_id = $2 AND started_at >= $3`, [agentId, companyId, since]);
...
export async function detectLoop(ctx, companyId) {
    try {
        const since = new Date(nowTs() - 7 * DAY_MS).toISOString();
        const rows = await ctx.db.query(`SELECT agent_id, COUNT(*) as count FROM heartbeat_runs WHERE company_id = $1 AND started_at >= $2 GROUP BY agent_id ORDER BY count DESC LIMIT 1`, [companyId, since]);
        if (!rows.length)
            return { detected: false, agentName: null, runs7d: null };
        const top = rows[0];
        if (top.count < LOOP_RUN_THRESHOLD)
            return { detected: false, agentName: null, runs7d: top.count };
...
export function buildTopBlockers(issues, company) {
...
export async function buildAgentStatuses(ctx, agents, companyId) {
    const since = new Date(nowTs() - 7 * DAY_MS).toISOString();
    const results = [];
    for (const agent of agents) {
        ...
        let runs7d = 0;
        try {
            const rows = await ctx.db.query(`SELECT COUNT(*) as count FROM heartbeat_runs WHERE agent_id = $1 AND company_id = $2 AND started_at >= $3`, [agent.id, companyId, since]);
...
export async function buildOverview(ctx, companyId) {
...
export function overviewToSnapshot(overview) {
    return {
...
export function buildAgentContextSnapshot(overview) {
...
export async function autoPauseLoopAgent(ctx, companyId, agentId, runs7d) {
    try {
