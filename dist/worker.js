  ...
  async setup(ctx) {
    ctx.data.register(DATA_KEYS.overview, async (params) => {
        ...
        if (issue.status === "done") doneIssues += 1;
          ...
          if (topBlockers.length < 5) {
            topBlockers.push({
              ...
              title: issue.title,
              ageDays: issue.createdAt ? Math.max(0, Math.floor((now - new Date(issue.createdAt).getTime()) / 864e5)) : 0
            });
          }
      ...
      for (const agent of agents) {
        ...
        if (!agent.lastHeartbeatAt || heartbeatAt !== heartbeatAt || now - heartbeatAt > 14 * 864e5) {
          staleAgents += 1;
        }
